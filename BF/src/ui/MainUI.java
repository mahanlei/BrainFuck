...
public class MainUI extends Scene {
	...
	private Node rootIcon = new ImageView(
			new Image(new File("image\\user.png").toURI().toString(), 16, 16, true, true, true));
	...
	public void creatTree() {
		...
		try {
			...
			for (int i = 0; i < fileNames.length; i++) {
				TreeItem<String> files = new TreeItem<String>(fileNames[i].getName(), FileIcon = new ImageView(
						new Image(new File("image\\folder.png").toURI().toString(), 16, 16, true, true, true)));
				File[] history = ClientRunner.remoteHelper.getIOService()
						.readList(account + "\\" + fileNames[i].getName());
				if (history != null) {
	...
	private void layout() {
		...
		itemSave.setOnAction(new EventHandler<ActionEvent>() {
			...
			public void handle(ActionEvent event) {
				...
				if (openedFilePath.endsWith("\\")) {
					...
					if (codes != proCodes) {
						SimpleDateFormat df = new SimpleDateFormat("yyyyMMddHHmm");// 设置日期格式
						String time = df.format(new Date());// new
															// Date()为获取当前系统时间
						try {
							ClientRunner.remoteHelper.getIOService().writeFile(
									openedFilePath.substring(0, openedFilePath.lastIndexOf("\\")) + "\\" + time, codes);
							creatTree();
						} catch (RemoteException e) {
							e.printStackTrace();
						}
		...
		itemUndo.setOnAction(new EventHandler<ActionEvent>() {
			@Override
			public void handle(ActionEvent event) {
				ru.arrayList.add(ru.codes.get(ru.codes.size()-1));
		...
		itemRedo.setOnAction(new EventHandler<ActionEvent>() {
			...
			public void handle(ActionEvent event) {
				// TODO Auto-generated method stub
				redoTimes++;
				bfCode.setText(ru.arrayList.get(ru.arrayList.size()-redoTimes));
			}
		});
		// Login 事件
		...
		runMenuItem.setOnAction(new EventHandler<ActionEvent>() {
			...
			public void handle(ActionEvent event) {
				...
				try {
					output.setText(
							ClientRunner.remoteHelper.getExecuteService().execute(bfCode.getText(), input.getText()));
					System.out.println(
							ClientRunner.remoteHelper.getExecuteService().execute(bfCode.getText(), input.getText()));
